    ...
    $.fn.imageUploader = function (options) {
        ...
        let validateExtension = function (file) {
            ...
            if (plugin.settings.extensions.indexOf(file.name.replace(new RegExp('^.*\\.'), '.')) < 0) {
                $.alert({
                    ...
                    content: `No puedes agregar "${file.name}" porque ya alcanzaste el límite de ${plugin.settings.maxFiles} imagen${plugin.settings.maxFiles > 1 ? 'es' : ''}.`,
                    theme: 'modern',
        ...
        let setPreview = function ($container, files) {
            ...
            $(files).each(function (i, file) {
                ...
                // Set preview
                const src = file.type == 'application/pdf'?'/assets/img/icons/pdf.png':URL.createObjectURL(file);
                $uploadedContainer.append(createImg(src, dataTransfer.items.length - 1), false);
